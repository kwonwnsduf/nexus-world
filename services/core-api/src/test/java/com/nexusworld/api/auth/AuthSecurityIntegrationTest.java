...
class AuthSecurityIntegrationTest {
  ...
  void rejectsMissingTokenInvalidCredentialsAndInsufficientRole() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/auth/me").header("Authorization", "Bearer invalid.jwt.token"))
    ...
    mockMvc
        .perform(get("/api/v1/admin/access-check").header("Authorization", "Bearer " + token))
        .andExpect(status().isForbidden());

    String adminToken = login("admin", "nexus-world-local-admin");
    mockMvc
        .perform(get("/api/v1/admin/access-check").header("Authorization", "Bearer " + adminToken))
  ...
  void provenanceResourcesAreTraceableValidatedAndRoleProtected() throws Exception {
            ...
            .perform(
                post("/api/v1/provenance-links")
                    ...
                    .content(
                        """
                        {
                          "subjectType": "WORLD_VERSION",
                          "subjectId": "%s",
                          "propertyPath": "/labor/employmentRate",
                          "evidenceId": "%s",
                          "transformation": {"method": "weighted_mean"}
                        }
                        """
            ...
            post("/api/v1/provenance-links")
                ...
                .content(
                    """
                    {
                      "subjectType": "WORLD_VERSION",
                      "subjectId": "%s",
                      "propertyPath": "/labor/transitionLag",
                      "assumptionId": "%s"
                    }
                    """
    ...
    mockMvc
        .perform(
            get("/api/v1/provenance-links/" + linkId)
                .header("Authorization", "Bearer " + viewerToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.evidenceId").value(evidenceId));
    mockMvc
        .perform(
            get("/api/v1/provenance-links")
                .param("subjectType", "WORLD_VERSION")
  ...
  void economicCivilizationGraphEnforcesSemanticsPrivacyAndRoles() throws Exception {
    ...
    jdbc.update(
        "INSERT INTO world_versions (id,world_id,version_number) VALUES (?,?,?)",
        versionId,
        worldId,
        1);

    mockMvc
        .perform(get("/api/v1/ontology").header("Authorization", "Bearer " + adminToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.ontologyVersion").value("economic-civilization-v1"))
        .andExpect(jsonPath("$.entityTypes[?(@.code == 'COMPANY')]").exists())
        .andExpect(
            jsonPath("$.propertyTypes[?(@.entityType == 'COMPANY' && @.code == 'industryCode')]")
                .exists())
            ...
            .perform(
                post("/api/v1/world-versions/" + versionId + "/graph/entities")
                    ...
                    .content(
                        ...
                        {
                          "entityType": "DEMOGRAPHIC_COHORT",
                          "naturalKey": "kr:cohort:engineers",
                          "displayName": "Semiconductor engineers",
                          "attributes": {
                            "populationModelId": "kr-2025-v1",
                            "baseYear": 2025,
                            "weight": 12500
    ...
    String companyId = objectMapper.readTree(companyBody).get("id").asText(),
        cohortId = objectMapper.readTree(cohortBody).get("id").asText();
    ...
    mockMvc
        .perform(
            post("/api/v1/world-versions/" + versionId + "/graph/relationships")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"relationshipType\":\"EMPLOYS\",\"sourceEntityId\":\""
                        + companyId
                        + "\",\"targetEntityId\":\""
                        + cohortId
                        + "\",\"attributes\":{\"jobs\":4200}}"))
        .andExpect(status().isCreated())
        .andExpect(jsonPath("$.sourceEntityType").value("COMPANY"));
    mockMvc
        .perform(
            post("/api/v1/world-versions/" + versionId + "/graph/relationships")
                .header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"relationshipType\":\"LENDS_TO\",\"sourceEntityId\":\""
                        + companyId
                        + "\",\"targetEntityId\":\""
                        + cohortId
                        + "\"}"))
        ...
        mockMvc
            .perform(
                post("/api/v1/auth/refresh")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
                        objectMapper.writeValueAsString(new RefreshTokenRequest(oldRefreshToken))))
            .andExpect(status().isOk())
